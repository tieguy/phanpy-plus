...
- `post-card-layout.js` (pure) — `layoutCard(model, measureText, opts)` wraps the content into positioned lines (word wrap, hard break for over-wide tokens, paragraph gaps, a line cap with a trailing `[…]` line). Every card ends with a small muted footer, `FOOTER_TEXT` ("Quoted post anonymized to avoid pile-ons"), placed outside the line cap so it always appears. The measurer is injected so the layout is unit-tested in Node.
