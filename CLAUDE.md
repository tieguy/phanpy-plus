...
- `post-card-render.js` (browser only, lazy-imported) — `renderCardBlob(status, opts)` draws the layout on a Canvas 2D surface at the device pixel ratio, floored at 2x, and resolves `{ blob, altText, truncated }`. The card is `DEFAULT_WIDTH` (600px) wide; a caller-supplied `opts.width` is floored at `MIN_WIDTH` (240px).
